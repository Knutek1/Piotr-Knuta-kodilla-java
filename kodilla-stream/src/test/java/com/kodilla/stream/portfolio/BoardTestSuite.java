    ...
    void testAddTaskListAverageWorkingOnTask(){
        ...
        inProgressTasks.add(new TaskList("In progress"));
         long suma = project.getTaskLists().stream()
                ...
                .map(Task::getCreated)
                .filter(d-> ChronoUnit.DAYS.between(d , LocalDate.now())>0)
                .map(d->ChronoUnit.DAYS.between(d , LocalDate.now()))
                .reduce((long)0, (sum, current) -> sum + current);

        long amountOfTask = project.getTaskLists().stream()
                .filter(inProgressTasks::contains)
                .flatMap(tl -> tl.getTasks().stream())                         // [5]
                .map(Task::getCreated)
                .filter(d-> ChronoUnit.DAYS.between(d , LocalDate.now())>0)
        ...
        double averageDays = suma/amountOfTask;

            //Then
        Assertions.assertEquals(15,averageDays);
